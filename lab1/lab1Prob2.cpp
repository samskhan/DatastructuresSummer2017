...
//Precondition: f is a double representing feet no less than 0
//Postcondition: The retun valie is the feet f converted to meters.
{const double MIN_FEET = 0;
 assert(f>=MIN_FEET);
 return (f/3.28);
}
...
int main( )
{
    ...
    const char   HEADING3[]  = "   Feet"; // Heading for table's 1st column
    const char   HEADING4[]  = "Meters"; // Heading for table's 2nd column
    const char   LABEL3      =          'F'; // Label for numbers in 1st column
    const char   LABEL4      =          'M'; // Label for numbers in 2nd column
    const double TABLE_BEGIN1 =        10.00; // The table's first Feet unit.
    const double TABLE_END1   =         50.00; // The table's final Feet unit.
    const double TABLE_STEP1  =         2.00; // Increment between temperatures
    const int    WIDTH1       =            9; // Number chars in output numbers
    const int    DIGITS1      =            1; // Number digits right of decimal pt

    double value3;  // A value from the table's first column
    double value4;  // A value from the table's second column
    
    // Set up the output for fractions and print the table headings.
    setup_cout_fractions(DIGITS1);
    cout << "CONVERSIONS from " << TABLE_BEGIN1 << " to " << TABLE_END1 << endl;
    cout << HEADING3 << "           " << HEADING4 << endl;
    
    // Each iteration of the loop prints one line of the table.
    for (value3 = TABLE_BEGIN1; value3 <= TABLE_END1; value3 += TABLE_STEP1) 
    {   
        value4 = feet_to_meters(value3);
        cout << setw(WIDTH1) << value3 << LABEL3 << "  "; 
        cout << setw(WIDTH1) << value4 << LABEL4 << endl; 
    }
